...
class ivMapperXmlFile extends ivMapperXmlAbstract
{
	...
	public function find($id)
	{
		...
		if (isset($this->_collection[$sId])) {
			...
			} else if ('flv' == strtolower(ivFilepath::suffix($id))) {
				$item = new ivFileFlv($this);
			} else if (in_array( strtolower(ivFilepath::suffix($id)), array('mp4'))) {
			...
			$this->_initFlvProperties($item, $id);
	...
	private function _initFlvProperties(ivFile $record, $id)
	{
		if (!is_a($record, 'ivFileFlv')) {
			return;
		}

		$dim = flvdim($id);

		$record->width = $dim['width'];
		$record->height = $dim['height'];
	}
	...
	public function getRawId3Data(ivFileMP3 $record)
	{
		...
		$data = $getID3->analyze(ROOT_DIR . $record->getPrimary());
		$tags = array();
