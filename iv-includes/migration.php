...
if (count($nonDefaultThemes)) {
	foreach ($nonDefaultThemes as $nonDefaultTheme) {
		...
		$xml = ivXml::readFromFile($newConfigPath . $nonDefaultTheme . DS . 'themeConfig.xml', $fullOldPath . 'themeConfig.xml');
	...
	while (false !== ($file = readdir($handle))) {
		if (!in_array($file, array('.', '..'))) {
			...
			if (is_file($filepath) || is_link($filepath)) {
				if (!@unlink($filepath)) {
					migrationError("Cannot remove <b>iv-includes/config/</b> folder. Please delete the file manually.");
				}
			} else {
