...
function menu(){
	$('#menu').hide().prependTo('.ui-page-active .ui-content');
	$('.t').unbind('click').click(function() {
		if (e=$('#menu')) {
			if (!$(this).hasClass('disabled')) {
			$(this).toggleClass('open');
			if ($(this).hasClass('open')) { window.scrollTo(0,0) }
			e.slideToggle("medium");
			}
		} else {
...
$(function() {
	...
	$('<style>.gallery li { width: ' + 100/columns + '% }</style>').appendTo( 'head' );
	positionArrows();
 });
...
function setOptions() {
	window.options = {
		...
		getToolbar: function() {
			return '<div class="dark"><div class="iv-header"><h1 class="ui-title">' + $('#theTitle').text() +'</h1><a class="button ps-toolbar-close close" data-rel="dialog"></a><div class="ps-toolbar-previous"></div><div class="ps-toolbar-next"></div></div></div>';
		}
	};
}
...
function positionArrows() {
	...
	$('.ps-toolbar-previous, .ps-toolbar-next, i.prev a, i.next a').css('top', height/2);
}
